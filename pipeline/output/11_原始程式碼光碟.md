...
- 11.2　原始程式碼清單
  ...
  - 11.2.2　前端程式清單（React 18+ / Next.js / Material UI（MUI））
  - 11.2.3　程式統計摘要
- 11.3　API 清單
...
- `L0215P2010U-SOURCE/` — 光碟根目錄
  ...
  - `src/` — 原始程式碼
    - `backend/` — 後端服務（Python 3.11+ / FastAPI）
      ...
      - `tests/` — 單元測試與整合測試原始碼
    - `frontend/` — 前端原始碼（React 18+ / Next.js / Material UI（MUI））
...
系統共計 **10 大子系統（含 KB 知識庫子模組）、45 支程式**，對應 48 項功能需求（FR）。3 支程式因功能高度耦合採合併實作（詳見 #10 程式設計書 §1.3）：P-IVR-01 合併 FR-IVR-001 + 002；P-LLM-02 合併 FR-LLM-002 + 003；P-AGT-03 合併 FR-AGT-003 + 004。

### 1.2.1　後端程式清單（Python 3.11+ / FastAPI）
...
| P-QI-01 | 質檢資料匯入 | FR-QI-001 | M-QI-01 | `src/backend/services/qi_engine/ingest.py` | Python |
...
### 1.2.2　前端程式清單（React 18+ / Next.js / Material UI（MUI））
...
內部高頻通訊採 gRPC，以下為主要 .proto 服務定義：

| 服務名稱 | .proto 檔案 | 方法 | 說明 |
|---------|------------|------|------|
| LLMInference | llm_inference.proto | InferStream (server streaming) | LLM 推論串流回應 |
| STTStream | stt_stream.proto | RecognizeStream (bidirectional streaming) | 即時語音辨識串流 |
| TTSStream | tts_stream.proto | SynthesizeStream (server streaming) | 語音合成串流輸出 |
| AgentOrchestrator | agent_orchestrator.proto | ProcessQuery (unary) | AI-Agent 問答請求 |

完整 .proto 定義檔收錄於光碟 `src/backend/proto/` 目錄。
...
本清單採用下列版本標註方式，確保保固期結束後光碟內程式碼可重建：

- **系統軟體（OS/LTS）**：標註官方長期支援版本名稱，格式「X.Y LTS」（如 Ubuntu 22.04 LTS）
- **開源框架與引擎**：標註下限版本，格式「≥X.Y.Z」，表示該版本及以上任何穩定版本均相容（如 Python ≥3.11.0、FastAPI ≥0.110.0）
- **AI 模型**：標註官方發布版本名稱（如 Llama 3.1、Whisper large-v3），與官方 GitHub Release 對應
- **備案方案**：若原方案開發停止，備案方案均已驗證相容，可替換部署

### 1.4.1　AI 模型與推論引擎

| 元件名稱 | 版本 | 授權類型 | 用途 | 備註 |
|---------|------|---------|------|------|
| Llama 3.1 8B-Instruct | Llama 3.1 | Meta Community License | LLM 推論（AWQ 4-bit 量化，~8GB VRAM） | AD-004 |
| vLLM | ≥0.5.0 | Apache 2.0 | LLM 推論引擎（PagedAttention） | AD-004 |
| Whisper large-v3 | Whisper large-v3 | MIT License | 語音辨識模型 | AD-003 |
| faster-whisper | ≥1.0.0 | MIT License | CTranslate2 加速版 Whisper | AD-003 |
| Coqui XTTS-v2 | XTTS-v2 | MPL 2.0 | 語音合成模型 | Coqui AI 已停業，模型仍開源可用 |
| MeloTTS | ≥0.1.0 | MIT License | TTS 備案方案 | 備案引擎，已驗證可行 |
| Piper | ≥1.0.0 | MIT License | TTS 備案方案 | 備案引擎，已驗證可行 |
| multilingual-e5-large | multilingual-e5-large | MIT | 中文向量 Embedding（~1.3GB） | Microsoft Research，MTEB 多語言排行前列 |
| Silero VAD | ≥4.0.0 | MIT License | 語音活動偵測 | P-STT-03 |

### 1.4.2　資料庫與快取

| 元件名稱 | 版本 | 授權類型 | 用途 |
|---------|------|---------|------|
| PostgreSQL | 16 LTS | PostgreSQL License | 關聯式資料庫 |
| TimescaleDB | ≥2.13.0 | Apache 2.0 | 時序資料擴展（日誌/監控） |
| Qdrant | ≥1.7.0 | Apache 2.0 | 向量資料庫（HNSW 索引，Top-K <50ms） |
| Redis | 7.2 LTS | BSD 3-Clause | Session Cache / 對話上下文（7.2.x 採 BSD 3-Clause 授權，無傳染性風險） |

### 1.4.3　後端框架與通訊

| 元件名稱 | 版本 | 授權類型 | 用途 |
|---------|------|---------|------|
| Python | ≥3.11.0 | PSF License | 後端主要語言 |
| FastAPI | ≥0.110.0 | MIT License | REST API 框架 |
| PJSIP (pjsua2) | ≥2.13.0 | GPL 2.0 | SIP/RTP 通訊堆疊 |
| gRPC (grpcio) | ≥1.56.0 | Apache 2.0 | 內部服務間通訊 |
| Alembic | ≥1.12.0 | MIT License | 資料庫 Migration |
...
| 元件名稱 | 版本 | 授權類型 | 用途 |
|---------|------|---------|------|
| React | ≥18.0.0 | MIT License | 前端 UI 框架 |
| Next.js | ≥14.0.0 | MIT License | React SSR 框架 |
| Material UI（MUI） | ≥5.14.0 | MIT License | 企業級 UI 元件庫 |
| TypeScript | ≥5.0.0 | Apache 2.0 | 前端開發語言 |

### 1.4.5　監控與部署

| 元件名稱 | 版本 | 授權類型 | 用途 |
|---------|------|---------|------|
| Prometheus | ≥2.45.0 | Apache 2.0 | 指標收集與告警 |
| Grafana | ≥10.0.0（OSS） | AGPL 3.0 | 視覺化監控儀表板 |
| Docker | ≥24.0.0 | Apache 2.0 | 容器化部署 |
| Docker Compose | ≥2.20.0 | Apache 2.0 | 多容器編排（AD-006） |
| NVIDIA Container Toolkit | ≥12.3.0 | Apache 2.0 | GPU 容器支援 |
| Nginx | ≥1.25.0 | BSD 2-Clause | 反向代理 / 負載均衡 |
| Keepalived | ≥2.2.0 | GPL 2.0 | VIP 高可用切換（AD-005） |

### 1.4.6　GPL 授權傳染性說明

PJSIP（GPL 2.0）：pjsua2 Python binding 屬於動態連結（dynamic linking），本案 SIP 通訊模組獨立編譯為單獨程序，不與其他子系統程式碼合併發布。依 GPL-2.0 授權慣例，動態連結之傳染性判定仍有法律爭議，為審慎起見，SIP 通訊模組程式碼將依 GPL-2.0 條款一併開放原始碼。Keepalived 為獨立系統服務，同理不影響應用程式授權。Grafana OSS 版（AGPL 3.0）為獨立部署的監控工具，不與應用程式碼連結。授權合規性分析詳見 `licenses/COMPLIANCE.md`。
...
| multilingual-e5-large | ~1.3 GB | `models/embedding/multilingual-e5-large/` | 隨光碟附校驗碼 |
...
| `tra-keepalived` | `osixia/keepalived` | HA VIP 管理（AD-005） |
| `tra-alembic` | `python:3.11-slim` | 資料庫遷移工具（Alembic） |

> 上表列出 16 個主要服務映像，另有 Keepalived（HA VIP 管理）及 Alembic（資料庫遷移工具）2 個輔助映像，合計 18 個映像。

### 1.6.2　Docker Compose 配置
...
| `deploy/healthcheck.sh` | 全服務健康檢查（詳見 #07 §3.2 告警與監控操作） |
